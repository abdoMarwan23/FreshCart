...
export default function About() {
    ...
    return (
        <>
            <div className="card flex justify-center items-center flex-col border-none px-3 pb-5 lg:pl-11 lg:h-screen h-full  pt-7  ">
                <h1 style={{ color: 'var(--text-color-title)' }} className="text-5xl  font-bold   mb-14 " >ABOUT US</h1>
                <div className="container lg:pl-0 ">
                    ...
                    <div className="flex items-center justify-center lg:flex-row flex-col pt-9 gap-y-3 ">
                        <div className=" sm:w-80 xl:w-[28rem] ">
                            <img className="" src={teamPic} alt="" />
                        </div>
                        <div className=" lg:pl-16 lg:text-left  lg:mx-0 mx-auto">
                            ...
                            {showMore && <p style={{ color: 'var(--text-color-title)' }} className=" pt-2 sm:w-[32rem]" >With a commitment to sustainability and community, FreshCart ensures that every purchase you make contributes to a greener future. We prioritize transparency and ethical sourcing, so you can shop with confidence knowing you're making a positive impact.</p>}
                            <button onClick={() => setShowMore(!showMore)} className='flex items-center  justify-center mx-auto rounded-md bg-[#359751] text-white hover:bg-[#289045] mt-5 px-6 py-2.5 font-semibold hover:shadow-lg hover:drop-shadow transition duration-200' >Read {!showMore ? "More" : "Less"}</button>
                        </div>
                    </div>
                </div>
            </div>
